...
for record in cleaned_json:
    ...
    for key, value in record.items():
        if key.startswith('tunes_'):
            parts = key.split('_')

            if parts[-1].isdigit() and parts[0] == 'tunes':
                print(parts)
                index = f'tunes_settings_{int(parts[-1])}'
                sub_key = "_".join(parts[0:-1])
                if index not in tunes_data:
                    tunes_data[index] = {}
                tunes_data[index][sub_key] = value
        else:
            tunes_data[key] = value
    grouped_json.append(tunes_data)
